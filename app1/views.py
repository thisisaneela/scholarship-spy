...
def my_recommendations(request):
    ...
    if my_personal_statement!=None:
        if len(my_personal_statement) < 250:
            ...
            text = Clean(my_personal_statement)
            embeddings=word_embeddings()
            vec_per_count=Vectorize(text, embeddings)
            ...
            scholarships_list=[]
            for i in result:
                scholarships_list.append(df['UniversityName'][result[i]])
            
            
            #scholarships_list = Scholarship.objects.all()[:5]
